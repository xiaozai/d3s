...
class DepthSegm(BaseTracker):
    ...
    def update_dal_longterm_params(self, depth):
    ...
    ''' Song : one pass track '''
    def one_pass_track(self, color, depth, target_scale):

        im, dp = numpy_to_torch(color), numpy_to_torch(depth)

        # Get sample
        sample_pos = copy.deepcopy(self.pos)
        sample_scales = target_scale * self.params.scale_factors
    ...
    def track(self, image):
        ...
        if self.redetection_mode:
            ...
            self.target_scale_redetection=min(self.target_scale_redetection, 5*self.first_target_scale) # 2*self.first_target_scale
        ...
        if self.uncert_score < self.params.tracking_uncertainty_thr \
           ...
           and self.valid_d:
            ...
            # Compute scores
            scores_raw = self.apply_filter(test_x_rgb) # only layer3 feature from ResNet50
            translation_vec, scale_ind, s, flag = self.localize_target(scores_raw)
    ...
    def init_segmentation(self, color, depth, bb, init_mask=None):
        ...
        self.init_train_feat_segm_rgb = train_feat_segm_rgb  # Song : we use the initial train_feat_segm_rgb + temporay_train_feat_rgb
