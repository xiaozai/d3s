...
class DepthSegmST(BaseTracker):
    ...
    def outliers_remove_by_depth(self, depth_pixels, max_deviations=1, num_bins=30, hist_height=0, select_peak='closet'):
        ...
        if hist_height > 0:
            peaks, peaks_properties = find_peaks(depth_hist, height=num_pixels/hist_height)
        else:
        ...
        if len(peaks) >= 2 and (not math.isnan(self.target_depth)):

            if select_peak == 'closest':
    ...
    def track(self, image):
        ...
        if flag == 'not_found':
            ...
            '''Re-detection'''
            for redetect_factor in [1.2, 1.4, 1.6, 2, 3]:
                print('search redetct_factor : ', redetect_factor)
        ...
        if self.segmentation_task or (
            ...
            if pred_segm_region is None:
                ...
                ''' Get target depth and post-processing together ????
                if we want to do here, we need to convert mask back to image coordinates
                '''
                # get target depth according to the new bbox
    ...
    def init_segmentation(self, color, depth, bb, init_mask=None, raw_depth=None):
        ...
        ''' Song, Preprocessing for long-tail object initial object '''
        if raw_depth is not None:
        ...
        ''' Song, Pre-processing the initial mask '''
        if raw_depth is not None:
    ...
    def segment_target(self, color, depth, pos, sz, raw_depth=None):
        ...
        if patch_raw_d is not None:
            new_mask = self.outliers_remove_by_depth(mask*patch_raw_d, max_deviations=0.8, num_bins=20, hist_height=10, select_peak='first')
        ...
        if len(cnt_area) > 0 and len(contours) != 0 and np.max(cnt_area) > 1000:
            ...
            ''' Song, if we want, we need to project mask back to the image coordinates '''
            # mask = mask / _f + + np.array([pos[1].item(), pos[0].item()])
