...
class DepthSegmST(BaseTracker):
    ...
    def outliers_remove_by_depth(self, depth_pixels, max_deviations=1, num_bins=30):
        ...
        if self.segmentation_task or (
            ...
            if pred_segm_region is None:
                ...
                if (not math.isnan(new_target_depth)) and (not math.isnan(self.target_depth)):
                    ...
                    if target_depth_flag > 0.35 and self.target_depth > 1000 and new_target_depth > 1000:
    ...
    def init_segmentation(self, color, depth, bb, init_mask=None, raw_depth=None):
        ...
        train_feat_d = self.segm_net.extract_depth_backbone_features(init_patch_d)
        train_feat_segm_d = [feat for feat in train_feat_d.values()]
        test_feat_segm_d = [feat for feat in train_feat_d.values()]
        ...
        if init_mask is None:
            ...
            while iters < 1:
                ...
                segm_pred = self.segm_net.segm_predictor(test_feat_segm_rgb, test_feat_segm_d,
                ...
                mask = F.softmax(segm_pred, dim=1)[0, 0, :, :].cpu().detach().numpy()
    ...
    def segment_target(self, color, depth, pos, sz):
        ...
        test_feat_d = self.segm_net.extract_depth_backbone_features(patch_gpu_d)

        # prepare features in the list (format for the network)
        test_feat_segm_rgb = [feat for feat in test_feat_rgb.values()]
        test_feat_segm_d = [feat for feat in test_feat_d.values()]
        ...
        # softmax on the prediction (during training this is done internaly when calculating loss)
        # take only the positive channel as predicted segmentation mask
        mask = F.softmax(segm_pred, dim=1)[0, 0, :, :].cpu().detach().numpy() # [1,2,384, 384] -> [384,384]
