...
def run(settings):
    ...
    settings.env.images_dir = os.path.join(settings.env.workspace_dir, 'images_8ACNet_MP_woBG')
    if not os.path.isdir(settings.env.images_dir):
        os.mkdir(settings.env.images_dir)

    # Train datasets
    vos_train = Vos_rgbd(split='train')

    # Validation datasets
    vos_val = Vos_rgbd(split='val')
    ...
    dataset_train = depth_segm_sampler.DepthSegmSampler([vos_train], [1],
    ...
    dataset_val = depth_segm_sampler.DepthSegmSampler([vos_val], [1], samples_per_epoch=10 * settings.batch_size, max_gap=50,
    ...
    net = segm_models.depth_segm_attention02_8ACNet_MP_woBG_resnet50(backbone_pretrained=True, topk_pos=settings.segm_topk_pos,
    ...
    actor = actors.DepthSegmActor_MultiPred(net=net, objective=objective, loss_weights=[1.0, 0.5, 0.5, 0.5])
