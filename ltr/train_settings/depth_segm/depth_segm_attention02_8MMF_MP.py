...
def run(settings):
    ...
    settings.env.images_dir = os.path.join(settings.env.workspace_dir, 'images_rgbd2_8MMF_MP')
    ...
    net = segm_models.depth_segm_attention02_8MMF_MP_resnet50(backbone_pretrained=True, topk_pos=settings.segm_topk_pos,
    ...
    actor = actors.DepthSegmActor_MultiPred(net=net, objective=objective, loss_weights=[1.0, 0.1, 0.1, 0.1])
