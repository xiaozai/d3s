...
class SegmNet_TREG(nn.Module):
    """ Network module for IoU prediction. Refer to the paper for an illustration of the architecture.

    Song : Use TREG's target-aware transformer to replace the original similarity_segmentation
    """
    def __init__(self, segm_input_dim=(128,256), segm_inter_dim=(256,256), segm_dim=(64, 64), mixer_channels=2, topk_pos=3, topk_neg=3):
        super().__init__()
        ...
        self.conv1x1 = nn.Conv2d(64, 64, kernel_size=1, stride=1,padding=0, bias=True),
        self.layer_norm = nn.LayerNorm([64, 24, 24])
    ...
    def similarity_segmentation(self, f_test, f_train, mask_pos, mask_neg):
        '''Song's comments:
            ...
            if we use the transformer to estimate the pos_map and neg_map ???
            f_train + mask_pos == f_train + PrROI ???

                step 1 : f_train + mask_pos --> f_template    Question : will lose the background senmentatic features ?
                step 2 : (Q*K)*V
        '''

        ''' Alan's Code , cosine similarity
        # first normalize train and test features to have L2 norm 1
        # cosine similarity and reshape last two dimensions into one
        sim = torch.einsum('ijkl,ijmn->iklmn',
                           F.normalize(f_test, p=2, dim=1),
        ...
        ''' TREG's code, (Q*K)*V + F_test '''
        f_template = f_train * mask_pos                        # [1, 64, 24, 24]
        print('f_template:',f_template.shape)
        qk = torch.matmul(F.normalize(f_test, p=2, dim=1),
                          F.normalize(f_template, p=2, dim=1)) # [1, 64, 24, 24]
        # f_div_C = F.softmax(f, dim=-1)
        norm = 1.0 / (f_template.shape[2]*f_template.shape[3]) # [1, 64, 24, 24]
        norm_qk = qk * norm
        norm_qk = F.softmax(norm_qk, dim=1)                    # [1, 64, 24, 24]
        qkv = torch.matmul(norm_qk, f_template)                # [1, 64, 24, 24]
        qkv = self.conv1x1(y) + f_test                         # [1, 64, 24, 24] ?????
        prob = self.layer_norm(qkv)                            # [1, 64, 24, 24]
        prob = F.softmax(prob, dim=1)                         # [1, 64, 24, 24]

        pos_map = torch.mean(torch.topk(prob, self.topk_pos, dim=1).values, dim=1)                # [1, 1, 24, 24]
        neg_map = torch.mean(torch.topk(prob, self.topk_pos, dim=1, largest=False).values, dim=1) # [1, 1, 24, 24]
        return pos_map, neg_map
