...
class DepthAttn(nn.Module):
    ...
    def forward(self, f_d):
        ...
        f_d = self.conv(f_d)
        f_d, _ = self.channel_attn(f_d) # [B, C, H, W]
        attn_d = self.spatial_attn(f_d) # [B, 1, H, W]
        return attn_d
...
class DWNet(nn.Module):
    ...
    def forward(self, f_rgb, attn_d):
        ...
        f_rgb, attn_rgb_ch = self.channel_attn_rgb(f_rgb) # [B, C, H, W]
        attn_rgb_sp = self.spatial_attn_rgb(f_rgb)        # [B, 1, H, W]
        f_rgb = f_rgb * attn_rgb_sp + f_rgb
        ...
        return f_rgbd, (attn_rgb_ch*attn_rgb_sp, attn_d)
...
class SegmNet(nn.Module):
    ...
    def forward(self, feat_test, feat_test_d, feat_train, feat_train_d, mask_train, test_dist=None, debug=False):
        ...
        if not debug:
            ...
            return (out0, out1, out2, out3), (atnn00_rgb, attn01_rgb, attn02_rgb, attn03_d)
