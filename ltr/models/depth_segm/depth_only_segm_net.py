...
class DepthOnlySegmNet(nn.Module):
    ...
    def forward(self, feat_test_rgb, feat_test_d, feat_train_rgb, feat_train_d, mask_train, test_dist=None, debug=False):
        ...
        mask_pos = F.interpolate(mask_train[0], size=(f_test_d.shape[-2], f_test_d.shape[-1])) # [1,1,384, 384] -> [1,1,24,24]
        mask_neg = 1 - mask_pos
        ...
        dist = F.interpolate(test_dist[0], size=(f_test_d.shape[-2], f_test_d.shape[-1]))                 # [B, 1, 24, 24]

        # mix # torch.unsqueeze(pred_sm_rgb[:, :, :, 0], dim=1),
        #     # torch.unsqueeze(pred_pos_rgb, dim=1),
        segm_layers = torch.cat((torch.unsqueeze(pred_sm_d[:, :, :, 0], dim=1), # depth may mislead
        ...
        out = self.post2(F.upsample(self.d2(feat_test_d[2]) + self.s2(out), scale_factor=2))
        out = self.post1(F.upsample(self.d1(feat_test_d[1]) + self.s1(out), scale_factor=2))
        out = self.post0(F.upsample(self.d0(feat_test_d[0]) + self.s0(out), scale_factor=2))

        if debug:
            return out, (pred_sm_d, pred_sm_d)
        else:
            return out
