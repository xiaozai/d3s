...
class DepthSegmNetAttention(nn.Module):
    def __init__(self):
        ...
        self.transformers = nn.ModuleList([Transformer(config0, (192, 192), 4, True),
                                           Transformer(config1, (96, 96), 16, True),
                                           Transformer(config2, (48, 48), 32, True),
                                           Transformer(config3, (24, 24), 64, True, mask_pool=True)])

        self.ACNet_layers = nn.ModuleList([ACNet(segm_input_dim[0], segm_inter_dim[0], segm_inter_dim[0]),
                                           ACNet(segm_input_dim[1], segm_inter_dim[1], segm_inter_dim[1]),
                                           ACNet(segm_input_dim[2], segm_inter_dim[2], segm_inter_dim[2]),
                                           ACNet(segm_input_dim[3], segm_inter_dim[3], segm_inter_dim[3])])
    ...
    def process(self, pre_out, test_rgb, test_d, train_rgb, train_d, mask_train, layer=0):
        ...
        f_test_rgbd = self.ACNet_layers[layer](f_test_rgb, f_test_d)              # BxCxHxH + BxCxHxW => BxCxHxW
        f_train_rgbd = self.ACNet_layers[layer](f_train_rgb, f_train_d)
        ...
        if layer == 3:
            ...
            attn_rgbd, attn_weights = self.transformers[layer](f_test_rgbd)     # [B, Patches, C=768], attn_weights, [B, heads=12, patches, headsize=64]
            n_patches = attn_rgbd.shape[1]                                      # for each image, 16 patches, 64 patches, 256 patches
