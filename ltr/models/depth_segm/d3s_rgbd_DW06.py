...
class SegmNet(nn.Module):
    ...
    def __init__(self, mixer_channels=5, topk_pos=3, topk_neg=3):
        super().__init__()
    ...
    def forward(self, feat_test, feat_test_d, feat_train, feat_train_d, mask_train, test_dist=None, debug=False):
        ...
        # Depth Cosine mask
        f_test_d = F.interpolate(feat_test_d, size=(f_test.shape[-2], f_test.shape[-1]))
        f_train_d = F.interpolate(feat_train_d, size=(f_train.shape[-2], f_train.shape[-1]))
        ...
        if test_dist is not None:
            ...
            segm_layers = torch.cat((torch.unsqueeze(pred_sm[:, :, :, 0], dim=1),
                                     ...
                                     torch.unsqueeze(pred_sm_d[:, :, :, 0], dim=1),
        ...
        if not debug:
            ...
            return (out0, out1, out2, out3), (attn00, attn01, attn02, torch.unsqueeze(pred_pos_d, dim=1))
