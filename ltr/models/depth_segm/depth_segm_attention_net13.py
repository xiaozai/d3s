    ...
    def forward(self, hidden_states_q, hidden_states_k=None, hidden_states_v=None, mask=None):
        ...
        if mask is not None:
            if torch.__version__ =='1.1.0':
                mask = torch.tensor(mask, dtype=torch.uint8).to(attention_scores.device) # .to('cuda')
            else:
...
class CrossAttentionTransformer(nn.Module):
    ...
    def forward(self, q_input, kv_input, mask=None):
        ...
        if mask is not None:
            ...
            if self.use_target_sz:
                sz_mask = torch.ones(mask.shape[0], 1, 1).to(q_embeddings.device) # .to('cuda')
...
class DepthSegmNetAttention(nn.Module):
    def __init__(self):
        ...
        config0 = get_config(size=(8, 8)) # 192, 8*8, 24*24 patches
        config1 = get_config(size=(8, 8)) # 96*96 -> 12*12 patches
        config2 = get_config(size=(8, 8)) # 48*48 -> 6*6 patches
        config3 = get_config(size=(4, 4)) # 24x24 -> 6*6?
    ...
    def attn_module(self, pre_attn, feat_test_rgb, feat_test_d, feat_train_rgb, feat_train_d, mask_train=None, layer=0):
        ...
        residual_att = torch.eye(patch_q, m=patch_kv).to(attn_weights.device) # .to('cuda')                # [P_q, P_kv]
