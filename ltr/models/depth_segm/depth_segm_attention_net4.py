...
class Decoder(nn.Module):
    ...
    def forward(self, query, key, value):
        attn_weights = []
        for layer_block in self.layer:
            query, weights = layer_block(query, key, value)
            if self.vis:
                attn_weights.append(weights)
...
class DepthSegmNetAttention04(nn.Module):
    ...
    def init_mask(self, test_dist, feat_test_rgb, feat_test_d, feat_train_rgb, feat_train_d, mask_train, layer=3):
        ...
        template = torch.cat((f_train_rgb, f_train_d, f_train_rgb*bg_mask, f_train_d*bg_mask), dim=2) # Bx64x24x24 -> Bx64x(24*4)x24
