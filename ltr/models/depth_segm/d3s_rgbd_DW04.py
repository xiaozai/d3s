...
class DepthAttn(nn.Module):
    ...
    def forward(self, f_d):
        ...
        f_d = self.conv(f_d)
        f_d, attn_dc = self.channel_attn(f_d)
        f_d = f_d * attn_dc
        attn_ds = self.spatial_attn(f_d)
        return attn_ds
...
class DWNet(nn.Module):
    ...
    def forward(self, f_rgb, f_d, attn_d):
...
class SegmNet(nn.Module):
    ...
    def forward(self, feat_test, feat_test_d, feat_train, feat_train_d, mask_train, test_dist=None, debug=False):
        ...
        f_test, attn03 = self.rgbd_fusion3(f_test, feat_test_d, attn_d_test)
        f_train, _ = self.rgbd_fusion3(f_train, feat_test_d, attn_d_train)
        ...
        f_test_rgbd2, attn02 = self.rgbd_fusion2(self.f2(feat_test[2]), self.d2(feat_test_d), attn_d_test)
        out2 = self.post2(F.upsample(self.m2(f_test_rgbd2) + self.s2(out3), scale_factor=2))

        f_test_rgbd1, attn01 = self.rgbd_fusion1(self.f1(feat_test[1]), self.d1(feat_test_d), attn_d_test)
        out1 = self.post1(F.upsample(self.m1(f_test_rgbd1) + self.s1(out2), scale_factor=2))

        f_test_rgbd0, attn00 = self.rgbd_fusion0(self.f0(feat_test[0]), self.d0(feat_test_d), attn_d_test)
