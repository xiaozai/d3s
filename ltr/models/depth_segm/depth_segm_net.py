...
class DepthSegmNet(nn.Module):
    ...
    def forward(self, feat_test_rgb, feat_test_d, feat_train_rgb, feat_train_d, mask_train, test_dist=None, debug=False):
        ...
        if debug:
            return out3, (pred_pos_rgb.cpu().detach().numpy().squeeze(),
                          pred_neg_rgb.cpu().detach().numpy().squeeze(),
                          pred_pos_d.cpu().detach().numpy().squeeze(),
                          pred_neg_d.cpu().detach().numpy().squeeze())
        else:
            return out3
