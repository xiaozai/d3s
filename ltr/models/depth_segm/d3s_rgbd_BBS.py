...
class DWNet_with_attnd(nn.Module):
    ...
    def forward(self, f_rgb, attn_d):
        ''' same as DWNet, but with attn_d '''
        f_rgb = self.channel_attn(self.conv_rgb(f_rgb))
        attn_rgb = self.spatial_attn(f_rgb)
...
class DWNet(nn.Module):
    def __init__(self, rgb_dims, d_dims, output_dims):
        ...
        self.channel_attn_d = channel_attention(output_dims, reduction_ratio=output_dims//4) # 4, 16, 32, 64
...
class DepthNet(nn.Module):
    def __init__(self, input_dim=1, inter_dim=(4, 16, 32, 64)):
        ...
        self.conv0 = conv131_layer(input_dim, inter_dim[0])    # B, 16, 192, 192
        self.conv1 = conv131_layer(inter_dim[0], inter_dim[1]) # B, 16, 96, 96
        self.conv2 = conv131_layer(inter_dim[1], inter_dim[2], kernel_size=3, stride=1, padding=1) # B, 32, 96, 96
        self.conv3 = conv131_layer(inter_dim[2], inter_dim[3], kernel_size=3, stride=1, padding=1) # B, 64, 96, 96
    ...
    def forward(self, dp):
        ...
        feat2 = self.conv2(feat1) # 96, 96
        feat3 = self.conv3(feat2) # 96, 96
        return feat3
...
class SegmNet(nn.Module):
    ...
    def __init__(self, mixer_channels=4, topk_pos=3, topk_neg=3):
        ...
        self.t1 = conv(segm_inter_dim[1], segm_inter_dim[0], kernel_size=1, padding=0)
        self.t2 = conv_no_relu(segm_inter_dim[0], 1)
    ...
    def forward(self, feat_test, feat_test_d, feat_train, feat_train_d, mask_train, test_dist=None, debug=False):
        ''' Inspired by TIP , Bifurcated Backbone Strategy for RGB-D Salient Object Detection
            we use a simplified version of BBS

            Teacher Feature group : Layer3, Layer2
            Student Feature group : Layer1, Layer0
        '''
        ...
        init_segm = F.upsample(torch.cat((init_segm, 1 - init_segm), dim=1), scale_factor=4) # B, 2, 384, 384
