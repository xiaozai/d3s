...
class DepthSegmNetAttention07(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.rgbd_transformers = nn.ModuleList([CrossAttentionTransformer(config, (feat_sz[0]*2, feat_sz[0]), segm_inter_dim[0], vis=True),  # 192x192x4 -> 16x16x4 patches
                                                CrossAttentionTransformer(config, (feat_sz[1]*2, feat_sz[1]), segm_inter_dim[1], vis=True),  # 96x94x16  -> 8x8x4 patches
                                                CrossAttentionTransformer(config, (feat_sz[2]*2, feat_sz[2]), segm_inter_dim[2], vis=True)]) # 48x48x32  -> 4x4x4 patches
    ...
    def rgbd_fusion(self, pre_out, feat_test_rgb, feat_test_d, feat_train_rgb, feat_train_d, mask_train, layer=0):
        ...
        out, attn_weights = self.rgbd_transformers[layer](template, search_region, mask=mask)      # [B, Patches, C], attn_weights, [B, heads=12, patches, headsize=64]
